...
const api = {
  ...
  asrRecognize: (options: AsrOptions): Promise<AsrResult> =>
    ipcRenderer.invoke("asr-recognize", options),
  generateVariants: (
    options: GenerateVariantsOptions,
  ): Promise<GenerateVariantsResult> =>
    ipcRenderer.invoke("generate-variants", options),
  testLlmProvider: (provider: LlmProvider): Promise<LlmTestResult> =>
    ipcRenderer.invoke("test-llm-provider", provider),
  testLlmProviders: (providers: LlmProvider[]): Promise<LlmTestResult[]> =>
    ipcRenderer.invoke("test-llm-providers", providers),
  setLlmRpmLimit: (rpm: number): Promise<number> =>
    ipcRenderer.invoke("set-llm-rpm-limit", rpm),
  getLlmRpmLimit: (): Promise<number> =>
    ipcRenderer.invoke("get-llm-rpm-limit"),
  selectOutputDir: (): Promise<string | null> =>
    ipcRenderer.invoke("select-output-dir"),
  exportVariants: (options: ExportOptions): Promise<ExportResult> =>
    ipcRenderer.invoke("export-variants", options),
  ...
  getAppSettingsPath: (): Promise<string> =>
    ipcRenderer.invoke("get-app-settings-path"),
  getLocalModelAdvice: (): Promise<any> =>
    ipcRenderer.invoke("get-local-model-advice"),
  ...
  onExportProgress: (
    ...
    const listener = (
      ...
      data: { current: number; total: number; detail?: string },
    ) => callback(data);
