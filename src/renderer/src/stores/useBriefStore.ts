...
export type TemplateId =
  "general" | "beauty" | "food" | "home" | "apparel" | "knowledge";
...
function feedbackScore(record: FeedbackRecord): number {
  return (
    safeMetric(record.threeSecondRate) * 0.2 +
...
export function summarizeModelTokenUsages(
  ...
  return mergeModelTokenUsages(
    ...records.map((record) => getUsageRecordModelUsages(record)),
  );
}

export function buildFeedbackInsights(records: FeedbackRecord[]): string {
  const usable = records.filter((record) =>
    String(record?.videoName || "").trim(),
  );
  if (usable.length === 0) return "";

  const averages = usable.reduce(
    (result, record) => ({
      threeSecondRate:
        result.threeSecondRate + safeMetric(record.threeSecondRate),
  ...
  const bestHooks = Array.from(hookGroups.entries())
    .map(([hookType, items]) => ({
      hookType,
      score:
        items.reduce((sum, item) => sum + feedbackScore(item), 0) /
        items.length,
      clickRate:
        items.reduce((sum, item) => sum + safeMetric(item.clickRate), 0) /
        items.length,
      conversionRate:
        items.reduce((sum, item) => sum + safeMetric(item.conversionRate), 0) /
        items.length,
    }))
  ...
  if (bestHooks.length > 0) {
    lines.push(
      `优先参考钩子：${bestHooks.map((item) => `${item.hookType}（点击 ${item.clickRate.toFixed(1)}%，转化 ${item.conversionRate.toFixed(1)}%）`).join("；")}。`,
    );
  }
  if (bestSamples.length > 0) {
    lines.push(
      `高表现样本：${bestSamples.map((item) => `${item.videoName}${item.hookType ? `/${item.hookType}` : ""}`).join("、")}。只学习其结构，不得编造原素材中不存在的信息。`,
    );
  }
  return lines.join("\n");
}
...
export const GROWTH_TEMPLATES: Array<{
...
}> = [
  {
    id: "general",
    name: "通用投流",
    description: "钩子、痛点、卖点、证据、逼单完整结构",
    defaults: { hookStrategies: ["pain", "benefit", "curiosity"] },
  },
  {
    id: "beauty",
    name: "美妆个护",
    description: "场景痛点、效果对比、成分背书、限时权益",
    defaults: {
      hookStrategies: ["pain", "benefit", "identity"],
      extraPrompt: "优先选择使用前后对比、成分与肤感相关原句。",
    },
  },
  {
    id: "food",
    name: "食品饮料",
    description: "口感场景、配料证据、价格权益、囤货理由",
    defaults: {
      hookStrategies: ["benefit", "price", "urgency"],
      extraPrompt: "优先选择口感、配料、食用场景和囤货权益相关原句。",
    },
  },
  {
    id: "home",
    name: "家清日用",
    description: "麻烦场景、效率提升、演示证据、立即下单",
    defaults: {
      hookStrategies: ["pain", "anti_common", "benefit"],
      extraPrompt: "优先选择清洁演示、效率对比和省时省力相关原句。",
    },
  },
  {
    id: "apparel",
    name: "服饰鞋包",
    description: "人群筛选、上身效果、材质版型、价格冲击",
    defaults: {
      hookStrategies: ["identity", "benefit", "price"],
      extraPrompt: "优先选择版型、身材适配、面料和穿搭场景相关原句。",
    },
  },
  {
    id: "knowledge",
    name: "知识服务",
    description: "结果利益、错误认知、方法证明、行动门槛",
    defaults: {
      hookStrategies: ["anti_common", "identity", "curiosity"],
      extraPrompt: "优先选择认知冲突、可验证结果和方法论相关原句。",
    },
  },
];
...
function migrateLegacy(): PersistedState | null {
  try {
    ...
    const legacyFeedback = Array.isArray(parsed?.feedback)
      ? parsed.feedback
      : [];
...
export const useBriefStore = create<BriefState>((set, get) => {
  ...
  const activeBrief =
    initial.briefs[initial.activeProductId] || createDefaultBrief();
  ...
  return {
    ...
    addFeedback: (record) => {
      const state = get();
      const nextFeedback = [
        { ...record, id: uid("feedback"), createdAt: Date.now() },
        ...state.feedback,
      ].slice(0, 300);
    ...
    recordUsage: (record) => {
      ...
      const previous = state.usage.filter(
        (item) => item.taskId !== record.taskId,
      );
      ...
      const nextRecord = existing
        ? {
            ...
            ...record,
            inputTokens:
              safeMetric(existing.inputTokens) + safeMetric(record.inputTokens),
            outputTokens:
              safeMetric(existing.outputTokens) +
              safeMetric(record.outputTokens),
            asrMinutes: Math.max(
              safeMetric(existing.asrMinutes),
              safeMetric(record.asrMinutes),
            ),
        ...
        : {
            ...record,
            modelUsages: incomingModelUsages,
            id: uid("usage"),
            createdAt: Date.now(),
          };
    ...
    renameProduct: (productId, name) => {
      ...
      const products = state.products.map((p) =>
        p.id === productId ? { ...p, name: name.trim() || p.name } : p,
      );
      set({ products });
      persist({
        products,
