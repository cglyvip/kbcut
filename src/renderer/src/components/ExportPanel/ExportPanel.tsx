...
export default function ExportPanel() {
  ...
  useEffect(() => {
    const cleanup = window.api.onExportProgress((data) =>
  ...
  const includedDuration = includedSegments.reduce(
    (sum, s) => sum + s.duration,
    0,
  );

  const enabledProviders = providers.filter(
    (p) => p.enabled && p.apiKey.trim() && p.baseUrl.trim() && p.model.trim(),
  );
  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
      ...
      if (result.usedFallback) {
        setError(
          result.notice || "全部大模型 API 失败，已使用本地兜底。请更换 API。",
        );
      ...
      const mapped = list.map((v: any) => ({
        ...
        name: v.name,
        strategy: v.strategy,
        sentences: (v.segments || []).map((seg: any) =>
          buildSentenceFromSeg(seg),
        ),
  ...
  const toggleEditing = (vi: number) => {
    setVariants((vs) =>
      vs.map((v, i) =>
        i === vi ? { ...v, editing: !v.editing, addingMode: false } : v,
      ),
    );
  };
  ...
  const toggleAddingMode = (vi: number) => {
    setVariants((vs) =>
      vs.map((v, i) => (i === vi ? { ...v, addingMode: !v.addingMode } : v)),
    );
  };
  ...
  const toggleWord = (vi: number, si: number, wi: number) => {
    setVariants((vs) =>
      vs.map((v, vIdx) => {
        ...
        return {
          ...v,
          sentences: v.sentences.map((s, sIdx) =>
            sIdx !== si
              ? s
              : {
                  ...s,
                  words: s.words.map((w, wIdx) =>
                    wIdx === wi ? { ...w, excluded: !w.excluded } : w,
                  ),
                },
          ),
        };
  ...
  const addSentence = (vi: number, seg: any) => {
    setVariants((vs) =>
      vs.map((v, i) => {
        if (i !== vi) return v;
        const exists = v.sentences.some(
          (s) =>
            s.start === seg.start &&
            s.end === seg.end &&
            s.originalText === seg.text,
        );
        if (exists) return v;
  ...
  const moveSentence = (vi: number, si: number, direction: "up" | "down") => {
    setVariants((vs) =>
      vs.map((v, i) => {
        ...
        [newSentences[si], newSentences[target]] = [
          newSentences[target]!,
          newSentences[si]!,
        ];
  ...
  const getVariantDuration = (v: VariantState) =>
    v.sentences.reduce(
      (sum, s) =>
        ...
        s.words
          .filter((w) => !w.excluded)
          .reduce((ws, w) => ws + (w.end - w.start), 0),
      0,
    );
  ...
  const handleExport = useCallback(async () => {
    ...
    try {
      const exportVariants = variants
        .map((v) => {
          ...
          return {
            ...
            strategy: v.strategy,
            segments: segs,
            totalDuration: segs.reduce(
              (sum: number, seg: any) => sum + seg.duration,
              0,
            ),
            targetAudience: v.targetAudience,
            abLabel: v.abLabel,
      ...
      if (result.files.length === 0) {
        ...
        setError(
          `部分导出失败：成功 ${result.files.length}/${exportVariants.length} 个`,
        );
      }
    } catch (e: unknown) {
      setError(e instanceof Error ? e.message : String(e));
      ...
      <div style={styles.settingsCard}>
        ...
        <div style={styles.section}>
          ...
          <div style={styles.apiSummary}>
            <div style={styles.apiLine}>
              主 API：{providers[0]?.name || "未配置"} /{" "}
              {providers[0]?.model || "-"}
            </div>
            <div style={styles.apiLine}>
              已启用 {enabledProviders.length} 个，候补{" "}
              {Math.max(0, providers.length - 1)} 个
            </div>
            <div style={styles.apiLine}>
              请在顶部“设置”中管理 API、测试连通与候补切换
            </div>
          </div>
            ...
            <div style={styles.modelUsageBox}>
              {modelUsages.map((usage) => (
                <div
                  ...
                  style={styles.modelUsageLine}
                >
                  实际模型：{usage.providerName} / {usage.model}；请求{" "}
                  {usage.requestCount} 次；输入{" "}
                  {usage.inputTokens.toLocaleString()} Token；输出{" "}
                  {usage.outputTokens.toLocaleString()} Token
                  {usage.estimated ? "（服务商未返回完整用量，含估算）" : ""}
                </div>
              ))}
            </div>
          )}
        ...
        <div style={styles.section}>
          ...
          <p style={styles.meta}>
            当前可用素材：{includedSegments.length} 段 /{" "}
            {includedDuration.toFixed(1)}s
          </p>
          <p style={styles.meta}>
            生成策略：通顺优先，拒绝流水账乱拼；结构=钩子→痛点→卖点→逼单
          </p>
          {diagnostics && (
            <div style={styles.diagnosticBox}>
              ...
              {diagnostics.suggestions.length > 0 && (
                <span>
                  建议：{diagnostics.suggestions.slice(0, 2).join("；")}
                </span>
              )}
            </div>
          )}
        ...
        <button
          style={{
            ...styles.generateBtn,
            ...(generating || includedSegments.length === 0
              ? styles.btnDisabled
              : {}),
          }}
          ...
          <>
            ...
            <div style={styles.field}>
              ...
              <div style={styles.dirRow}>
                ...
                <button style={styles.dirBtn} onClick={handleSelectDir}>
                  {outputDir ? "更换" : "选择"}
                </button>
              </div>
            </div>
            ...
            <div style={styles.exportOptions}>
              ...
              >
                ...
                <div style={styles.switchTextWrap}>
                  ...
                  <span style={styles.switchDesc}>
                    {enableSubtitle ? "导出视频将嵌入字幕" : "导出视频不带字幕"}
                  </span>
                </div>
              </div>
            </div>
            ...
            <p style={styles.meta}>
              当前待导出：{variants.length} 个变体
              {topFluencyOnly ? "（Top3 模式）" : ""}
            </p>
            <button
              style={{
                ...styles.exportBtn,
                ...(exporting || !outputDir ? styles.btnDisabled : {}),
              }}
            ...
            >
              {exporting
                ...
                : enableSubtitle
                  ? `开始导出 ${variants.length} 个（含字幕）`
                  : `开始导出 ${variants.length} 个（无字幕）`}
            </button>
          </>
        )}
        ...
        {exportResult && (
          ...
          >
            ...
            >
              {exportResult.files.length > 0
                ? `导出完成！成功 ${exportResult.files.length} 个`
                : "导出失败，未生成文件"}
            </p>
            {exportResult.files.length > 0 && (
              <button
                style={styles.openBtn}
                onClick={() => outputDir && window.api.openFolder(outputDir)}
              >
                打开文件夹
              </button>
            )}
                ...
                <div key={vi} style={styles.variantItem}>
                  ...
                  {(v.abLabel || v.targetAudience) && (
                    <div style={styles.variantTags}>
                      {v.abLabel && (
                        <span style={styles.abTag}>{v.abLabel}</span>
                      )}
                      {v.targetAudience && (
                  ...
                  {v.quality && (
                    <div style={styles.scoreGrid}>
                      <span>钩子 {v.quality.hook}</span>
                      <span>清晰 {v.quality.clarity}</span>
                      <span>痛点 {v.quality.pain}</span>
                      <span>卖点 {v.quality.sellingPoint}</span>
                      <span>证据 {v.quality.evidence}</span>
                      <span>转化 {v.quality.cta}</span>
                      <span>连贯 {v.quality.transition}</span>
                      <span>合规 {v.quality.compliance}</span>
                    </div>
                  )}
                  {v.pacingHints && v.pacingHints.length > 0 && (
                    <div style={styles.pacingText}>
                      字幕/画面节奏：{v.pacingHints.join("；")}
                    </div>
                  )}
                  {v.quality?.warnings && v.quality.warnings.length > 0 && (
                    <div style={styles.qualityWarning}>
                      质量/合规提醒：{v.quality.warnings.join("；")}
                    </div>
                  )}
                  ...
                  {v.editing ? (
                    <div>
                        ...
                        <div key={si} style={styles.sentenceBlock}>
                          <div style={styles.sentenceHeader}>
                            <span style={styles.sentenceTime}>
                              {formatTime(sent.start)}
                            </span>
                            <button
                              style={{
                                ...styles.moveBtn,
                                ...(si === 0 ? styles.moveBtnDisabled : {}),
                              }}
                              onClick={() =>
                                si > 0 && moveSentence(vi, si, "up")
                              }
                              disabled={si === 0}
                            >
                              上移
                            ...
                            <button
                              style={{
                                ...styles.moveBtn,
                                ...(si === v.sentences.length - 1
                                  ? styles.moveBtnDisabled
                                  : {}),
                              }}
                              onClick={() =>
                                si < v.sentences.length - 1 &&
                                moveSentence(vi, si, "down")
                              }
                            ...
                            {v.sentences.length > 1 && (
                              <button
                                style={styles.removeSentBtn}
                                onClick={() => removeSentence(vi, si)}
                              >
                                移除
                              </button>
                            )}
                          ...
                          <div style={styles.wordFlow}>
                            {sent.words.map((w, wi) => {
                              const includedCount = sent.words.filter(
                                (x) => !x.excluded,
                              ).length;
                      ...
                      <button
                        style={styles.addSentBtn}
                        onClick={() => toggleAddingMode(vi)}
                      >
                          ...
                          {allSentencePool.map((seg, idx) => {
                            ...
                            return (
                              <div
                                ...
                                style={{
                                  ...styles.poolItem,
                                  ...(exists ? styles.poolItemDisabled : {}),
                                }}
                                onClick={() => !exists && addSentence(vi, seg)}
                              >
                                <span style={styles.poolTime}>
                                  {formatTime(seg.start)}
                                </span>
                                <span style={styles.poolText}>{seg.text}</span>
                                {exists && (
                                  <span style={styles.poolTag}>已添加</span>
                                )}
                              </div>
                            );
                          })}
