...
export default function SettingsModal({ open, onClose }: SettingsModalProps) {
  ...
  const [tab, setTab] = useState<
    "llm" | "asr" | "asrGuide" | "export" | "local" | "about"
  >("llm");
  const [testingId, setTestingId] = useState<string | null>(null);
  const [testingAll, setTestingAll] = useState(false);
  const [msg, setMsg] = useState<string | null>(null);
  const [err, setErr] = useState<string | null>(null);
  const [localAdvice, setLocalAdvice] = useState<LocalModelAdviceView | null>(
    null,
  );
  const [localLoading, setLocalLoading] = useState(false);
  const [localErr, setLocalErr] = useState<string | null>(null);
  const [applyingId, setApplyingId] = useState<string | null>(null);
  const [asrModelInfo, setAsrModelInfo] = useState<AsrModelInfoView | null>(
    null,
  );
  ...
  const handleTestAll = useCallback(async () => {
    ...
    try {
      ...
      if (ok.length > 0) {
        ...
        setMsg(
          `✅ 可用 ${ok.length} 个，失败 ${bad.length} 个。已将成功 API 置顶。`,
        );
      } else {
      ...
      <div style={styles.modal} onClick={(e) => e.stopPropagation()}>
        <div style={styles.header}>
          <div>
            ...
            <p style={styles.sub}>
              配置会永久保存到本机，重启后自动恢复；导出分辨率可设置（保持比例）
            </p>
          </div>
          <button
        ...
        <div style={styles.tabs}>
          <button
            style={tab === "llm" ? styles.tabActive : styles.tab}
            onClick={() => setTab("llm")}
          >
            大模型 API
          </button>
          <button
            style={tab === "asr" ? styles.tabActive : styles.tab}
            onClick={() => setTab("asr")}
          >
            语音识别
          </button>
          <button
            style={tab === "asrGuide" ? styles.tabActive : styles.tab}
            onClick={() => setTab("asrGuide")}
          >
            识别教程
          </button>
          <button
            style={tab === "export" ? styles.tabActive : styles.tab}
          ...
          <button
            style={tab === "about" ? styles.tabActive : styles.tab}
