...
interface LocalModelTabProps {
  ...
  onApplyPreset: (
    rec: LocalModelAdviceView["recommendations"][number],
    asPrimary: boolean,
  ) => void;
}
...
export default function LocalModelTab({
  ...
  return (
    <div>
      <p style={styles.tip}>
        大多数用户本机没有本地模型。请先安装客户端并下载模型，再使用「一键填入」。仅填入
        API 地址无法直接运行。
      </p>
      ...
      {localLoading && (
        <p style={styles.tip}>
          正在检测硬件与本地推理服务（Ollama / LM Studio）...
        </p>
      )}
            ...
            <div style={styles.localHwGrid}>
              <div style={styles.localHwItem}>
                ...
                <div style={styles.localHwValue}>
                  {localAdvice.hardware.cpuModel}（
                  {localAdvice.hardware.cpuCores} 核）
                </div>
              </div>
              <div style={styles.localHwItem}>
                <div style={styles.label}>内存</div>
                <div style={styles.localHwValue}>
                  {localAdvice.hardware.totalMemGB}GB 总 /{" "}
                  {localAdvice.hardware.freeMemGB}GB 空闲
                </div>
              </div>
              <div style={styles.localHwItem}>
                ...
                <div style={styles.localHwValue}>
                  ...
                  {localAdvice.hardware.vramGB != null
                    ? `（约 ${localAdvice.hardware.vramGB}GB）`
                    : ""}
              ...
              <div style={styles.localHwItem}>
                ...
                <div style={styles.localHwValue}>
                  ...
                  {localAdvice.runtime.ollama.running
                    ? `运行中（${localAdvice.runtime.ollama.models.length} 个模型）`
                    : "未检测到"}
                  {" · "}
                  LM Studio:{" "}
                  {localAdvice.runtime.lmStudio.running ? "运行中" : "未检测到"}
                </div>
              </div>
            </div>
          </div>
            ...
            {(localAdvice.runtime.apps || []).map((app) => (
              ...
              >
                ...
                <div style={styles.actions}>
                  ...
                  <button
                    style={styles.miniBtn}
                    onClick={() => onOpenExternal(app.docsUrl)}
                  >
                    打开官网
                  </button>
                </div>
            ...
            {localAdvice.recommendations.map((rec) => (
              ...
              >
                <div style={styles.providerHeader}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={styles.switchDesc}>
                      ...
                      {rec.runtime
                        ? ` · ${rec.runtime === "ollama" ? "Ollama" : "LM Studio"}`
                        : ""}
                    </div>
                  </div>
                </div>
                ...
                <div style={styles.localPresetMeta}>
                  ...
                  {rec.downloadCommand && (
                    <div>下载命令：{rec.downloadCommand}</div>
                  )}
                ...
                <div style={styles.actions}>
                  {rec.modelPageUrl && (
                    <button
                      style={styles.miniBtn}
                      onClick={() => onOpenExternal(rec.modelPageUrl)}
                    >
                      打开模型下载页
                    </button>
                  )}
                  {rec.downloadCommand && (
                    <button
                      style={styles.miniBtn}
                      onClick={() =>
                        onCopyText(rec.downloadCommand || "", "下载命令")
                      }
                    >
                      复制下载命令
                    </button>
            ...
            <div style={styles.localTipsBox}>
              ...
              <ul style={styles.localTipsList}>
                {localAdvice.setupGuide?.map((tip, idx) => (
                  <li
                    key={`setup_${idx}_${tip.slice(0, 12)}`}
                    style={styles.localTipItem}
                  >
                    {tip}
                  </li>
                ))}
