...
async function runAsr(audioPath: string): Promise<AsrResult> {
  ...
  const ortWasmMjsPath = moduleRequire.resolve(
    "onnxruntime-web/ort-wasm-simd-threaded.asyncify.mjs",
  );
  const ortWasmPath = moduleRequire.resolve(
  ...
  const modelCacheDir =
    typeof workerData?.modelCacheDir === "string"
  ...
  if (!pipe) {
    const detail =
      mirrorError instanceof Error
        ? mirrorError.message
        : String(mirrorError || "未知错误");
    throw new Error(
      `本地 Whisper 模型下载/加载失败。已尝试镜像和官方源，请检查网络与磁盘空间。${detail}`,
    );
  }
    ...
    .map((chunk: any, index: number) => {
      ...
      const start =
        Number.isFinite(rawStart) && rawStart >= 0
          ? Math.min(rawStart, audioDuration)
          : 0;
      ...
      const endCandidate =
        Number.isFinite(rawEnd) && rawEnd > start ? rawEnd : estimatedEnd;
      const end = Math.max(
        start + 0.05,
        Math.min(audioDuration || start + 0.05, endCandidate),
      );
