...
export async function onlineAsr(
  audioPath: string,
  ...
  if (fileInfo.size > MAX_COMPATIBLE_UPLOAD_BYTES) {
    throw new Error(
      `在线识别音频约 ${(fileInfo.size / 1024 / 1024).toFixed(1)}MB，超过兼容上传上限 24MB。请缩短视频或切换本地识别。`,
    );
  }
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    ...
    try {
      ...
      if (response.status === 413) {
        throw new Error(
          "Whisper API 拒绝上传：音频文件过大（HTTP 413）。请缩短视频或切换本地识别。",
        );
      }
      if (
        includeWordTimestamps &&
        (response.status === 400 || response.status === 422) &&
        /timestamp[_-]?granular|unknown\s+(field|parameter)|unsupported\s+(field|parameter)/i.test(
          errorText,
        )
      ) {
      ...
      const retryable =
        isAbort ||
        /fetch failed|network|econnreset|socket|timeout|超时/i.test(lastError);
      if (!retryable || attempt >= 2) throw new Error(lastError);
  ...
  const wordItems = Array.isArray(data.words)
    ? data.words
    : rawSegments.flatMap((segment: any) =>
        Array.isArray(segment?.words) ? segment.words : [],
      );
  const rawWords: AsrWord[] = wordItems
    .map((word: any) => {
  ...
  const durationHint = finiteTime(
    data.duration,
    rawWords.reduce((max, word) => Math.max(max, word.end), 0),
  );
  const segments: AsrSegment[] = rawSegments
    .map((seg: any, index: number) => {
      const start = finiteTime(
        seg?.start,
        index > 0 ? finiteTime(rawSegments[index - 1]?.end, 0) : 0,
      );
      ...
      const segWords = rawWords.filter(
        (w) => w.start >= start - 0.05 && w.end <= end + 0.05,
      );
      return {
        start,
  ...
  const fullText = String(
    data.text || segments.map((segment) => segment.text).join(""),
  ).trim();
