...
function runFfmpeg(
  ...
  return new Promise((resolve, reject) => {
    ...
    const hardTimer = setTimeout(() => {
      ...
      reject(
        new Error(
          `${label} 超时（>${Math.round(timeoutMs / 1000)}s）。已强制结束导出进程。`,
        ),
      );
    }, timeoutMs);
    ...
    const stallTimer = setInterval(() => {
      ...
      const idle =
        Date.now() - (hasStartedEncoding ? lastProgressAt : lastActivityAt);
      ...
      if (idle > allowedIdle) {
        ...
        reject(
          new Error(
            `${label} 卡死（${Math.round(idle / 1000)}秒无进度，已编码 ${lastEncodedSec.toFixed(1)}s）。已强制结束。`,
          ),
        );
      }
    }, 2000);
...
async function probeMedia(videoPath: string): Promise<MediaProbe> {
  try {
    ...
    throw new Error(
      `无法读取源视频媒体信息：${err instanceof Error ? err.message : String(err)}`,
    );
  }
}
...
function buildScaleFilter(resolution: ExportResolution): string | null {
  ...
  const landscapeWidth =
    resolution === "720" ? 1280 : resolution === "1440" ? 2560 : 1920;
  const landscapeHeight =
    resolution === "720" ? 720 : resolution === "1440" ? 1440 : 1080;
...
async function exportByFilterGraph(
  ...
  try {
    onDetail?.(
      `${resolutionLabel(resolution)}编码 ${segs.length}段 · 片长约${expectedSec.toFixed(0)}s · 预计计算中`,
    );
    await runFfmpeg(ffmpegPath, args, {
      timeoutMs: Math.max(
        3 * 60 * 1000,
        Math.min(20 * 60 * 1000, expectedSec * 30 * 1000),
      ),
      ...
      onProgress: ({ encodedSec, speed, etaSec }) => {
        ...
        const etaText =
          etaSec == null ? "预计计算中" : `预计剩余 ${formatEta(etaSec)}`;
        const speedText = speed > 0 ? ` · ${speed.toFixed(2)}x` : "";
        onDetail?.(
          `编码 ${pct}% ${encodedSec.toFixed(1)}/${expectedSec.toFixed(1)}s${speedText} · ${etaText}`,
        );
      },
    });
    try {
...
async function exportSingleVariantFast(
  ...
  if (segs.length > 80) {
    throw new Error(
      `变体包含 ${segs.length} 个离散片段，超过稳定导出上限 80。请减少碎片化删词或重新生成，系统不会静默删除内容。`,
    );
  }
...
export async function exportVariants(
  ...
  const { videoPath, variants, outputDir, enableSubtitle, onProgress } =
    options;
  ...
  if (!(await pathExists(videoPath)))
    throw new Error(`源视频不存在: ${videoPath}`);
  ...
  onProgress?.(
    ...
    variants.length,
    hw
      ? `${resolutionLabel(resolution)} · 硬件加速 ${hw}`
      : `${resolutionLabel(resolution)} · 软件编码 ultrafast`,
  );

  for (let i = 0; i < variants.length; i++) {
    const variant = variants[i]!;
    const exportName =
      [variant.abLabel, variant.name].filter(Boolean).join("_") ||
      `变体${i + 1}`;
    ...
    try {
      ...
      hw = await exportSingleVariantFast(
        ...
        resolution,
        (detail) =>
          onProgress?.(i + 1, variants.length, `${label} · ${detail}`),
      );
