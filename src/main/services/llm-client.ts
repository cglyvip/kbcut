    ...
    const run = async () => {
      ...
      while (true) {
        ...
        this.tokenUsage = this.tokenUsage.filter(
          (item) => now - item.at < windowMs,
        );

        const gapWait =
          this.lastStartAt > 0 ? minGapMs - (now - this.lastStartAt) : 0;
        const usedTokens = this.tokenUsage.reduce(
          (sum, item) => sum + item.tokens,
          0,
        );
        const tokenSlotAvailable =
          this.tokenUsage.length === 0 ||
          usedTokens + estimatedTokens <= ESTIMATED_TPM;
        if (
        ...
        if (!tokenSlotAvailable && this.tokenUsage.length > 0) {
          waitMs = Math.max(
            waitMs,
            windowMs - (now - this.tokenUsage[0]!.at) + 30,
          );
        }
    ...
    while (true) {
      ...
      try {
        ...
        if (
          ...
          err.message === "__COOLDOWN_RETRY__" &&
          attempt < 10
        ) {
...
export function normalizeLlmApiUrl(baseUrl: string): string {
  return normalizeOpenAiCompatibleUrl(
    baseUrl,
    "chat/completions",
    "大模型 API",
  );
...
export function parseLlmCompletionPayload(
  ...
  if (finishReason === "length") {
    throw new Error(
      "API 响应被输出长度限制截断（finish_reason=length），请减少变体数量、缩短原视频文稿，或提高服务商输出 Token 上限",
    );
  }
  if (finishReason === "content_filter") {
    throw new Error(
      "API 响应被服务商内容审核拦截（finish_reason=content_filter）",
    );
  }
  ...
  const totalTokens = firstTokenValue(
    usage.total_tokens,
  ...
  return {
    content,
    model:
      String(data?.model || provider.model || "模型未知").trim() || "模型未知",
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      const retryable =
        ...
        /timeout|网络|fetch failed|econnreset|socket/i.test(lastError);
      ...
      console.warn(
        `[llm] retry after ${coolDownMs}ms: ${providerLabel(provider)} -> ${lastError}`,
      );
...
export async function callChatWithFailover(
  ...
  const list = (providers || []).filter(
    (p) => p && p.enabled !== false && p.apiKey && p.baseUrl && p.model,
  );
  if (list.length === 0) {
    throw new Error(
      "没有可用的大模型 API，请先添加并填写 API 地址 / Key / 模型",
    );
  }
  ...
  for (let i = 0; i < list.length; i++) {
    ...
    try {
      ...
      console.error(
        `[llm] provider failed: ${providerLabel(provider)} ->`,
        message,
      );
  ...
  const detail = failures
    .map(
      (f, idx) =>
        `${idx + 1}. ${providerLabel(f.provider)} (${maskKey(f.provider.apiKey)}): ${f.error}`,
    )
    .join("\n");

  throw new Error(`全部大模型 API 均失败，请检查/更换 API。\n${detail}`);
}
  ...
  try {
    ...
    const result = await callChatCompletionsDetailed(
      ...
      [
        {
          role: "system",
          content: "你是 API 业务测试助手。严格返回 JSON，不要 Markdown。",
        },
        {
          role: "user",
          content:
            '返回一个 JSON 数组，内容为 [{"ok":true,"message":"连接正常"}]。',
        },
      ],
      { temperature: 0, timeoutMs: 30000, maxRetries: 0 },
    );
    const latencyMs = Date.now() - started;
    const normalized = String(result.content)
      .trim()
      .replace(/^```json\s*/i, "")
      .replace(/\s*```$/, "");
...
export async function testLlmProviders(
  providers: LlmProvider[],
): Promise<LlmTestResult[]> {
