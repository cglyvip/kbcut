...
async function detectGpuWindows(): Promise<{
  name: string;
  vramGB: number | null;
  hasNvidia: boolean;
  hasAmd: boolean;
...
async function detectRuntime(): Promise<LocalRuntimeStatus> {
  ...
  const preferredRuntime: LocalRuntimeKind =
    ollamaRunning || !lmRunning ? "ollama" : "lmstudio";
  ...
  const apps: LocalRuntimeApp[] = [
    {
      ...
      description:
        "有 Windows 客户端。安装后会常驻托盘，支持一键下载模型，API 兼容 OpenAI。",
      installSteps: [
    ...
    },
    {
      ...
      description:
        "有完整桌面客户端，适合不习惯命令行的用户。在软件内搜索并下载模型，再开启本地服务器。",
      installSteps: [
...
function decideTier(h: HardwareInfo): LocalModelTier {
  if (h.totalMemGB >= 48 || (h.hasNvidia && (h.vramGB || 0) >= 16))
    return "ultra";
  if (h.totalMemGB >= 24 || (h.hasNvidia && (h.vramGB || 0) >= 10))
    return "high";
...
function buildRecommendations(
  ...
  const preferredApp =
    runtime.apps.find((a) => a.id === preferred) || runtime.apps[0]!;
  ...
  const installedLm: LocalModelRecommendation[] = (
    runtime.lmStudio.models || []
  )
    .slice(0, 4)
    .map((m, idx) => ({
  ...
  const installed: LocalModelRecommendation[] = (runtime.ollama.models || [])
    .slice(0, 4)
    .map((m, idx) => ({
      id: `installed_${idx}`,
      name: `本机已安装：${m}`,
  ...
  const primary = catalog
    .filter(
      (c) =>
        ...
        h.totalMemGB + 0.1 >= Math.max(8, c.minRamGB * 0.75),
    )
    .map((c, idx) => ({
      id: c.id,
      name: c.name,
      ...
      providerPreset: {
        ...
        apiKey,
        model:
          preferred === "ollama" ? c.model : runtime.lmStudio.models[0] || "",
      },
    }));
  ...
  if (all.length > 0 && !all.some((x) => x.recommended))
    all[0]!.recommended = true;
...
export async function getLocalModelAdvice(): Promise<LocalModelAdvice> {
  ...
  const preferredApp =
    runtime.apps.find((a) => a.id === runtime.preferredRuntime) ||
    runtime.apps[0];
  const topModel =
    recommendations.find((r) => r.recommended) || recommendations[0];
  ...
  if (!runtime.ollama.running && !runtime.lmStudio.running) {
    tips.push(
      "当前没有检测到本地模型服务。仅“一键填入”不够，需要先安装客户端并下载模型。",
    );
    tips.push(
      "优先推荐 Ollama：有 Windows 客户端，安装简单，模型下载命令也简单。",
    );
  ...
  if (runtime.ollama.running) {
    ...
    if (runtime.ollama.models.length === 0) {
      ...
      if (topModel)
        setupGuide.push(`在 PowerShell 执行：${topModel.downloadCommand}`);
    } else {
      tips.push(
        `Ollama 已安装 ${runtime.ollama.models.length} 个模型，可直接一键填入。`,
      );
    }
  }
  ...
  tips.push(
    "“一键填入”只会写入 API 地址；客户端未安装或模型未下载时，测试仍会失败。",
  );

  const summary =
    `本机约 ${hardware.totalMemGB}GB 内存 / ${hardware.cpuCores} 核，评级：${tierLabel(tier)}。` +
