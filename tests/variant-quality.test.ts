...
const segments: SimpleSegment[] = [
  { start: 0, end: 2, duration: 2, text: '为什么很多上班族每天收拾桌面还是很乱？' },
  { start: 2, end: 4, duration: 2, text: '东西反复找不到，既浪费时间又让人很烦。' },
  { start: 4, end: 6, duration: 2, text: '这款分区收纳盒拿取方便，能让桌面快速整齐。' },
  { start: 6, end: 8, duration: 2, text: '材质已经通过检测报告，很多用户实测后都在回购。' },
  { start: 8, end: 10, duration: 2, text: '现在点击链接领券下单，马上把桌面整理好。' }
]
...
describe('variant quality metadata', () => {
  it('turns historical delivery data into reusable model guidance', () => {
    const insights = buildFeedbackInsights([
      ...
      { id: '2', videoName: '悬念版B', hookType: '悬念型', threeSecondRate: 51, completionRate: 24, clickRate: 3.1, conversionRate: 1.2, spend: 200, createdAt: 2 }
    ])
  ...
  it('returns diagnostics, quality scores, A/B labels, and pacing hints without an API', async () => {
    ...
    expect(result.variants.every((variant) => typeof variant.quality?.total === 'number')).toBe(true)
    expect(result.variants.some((variant) => Boolean(variant.abLabel))).toBe(true)
    expect(result.variants.some((variant) => (variant.pacingHints?.length || 0) > 0)).toBe(true)
  })
