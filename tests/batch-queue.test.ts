...
beforeAll(async () => {
  ...
  useBatchStore = (await import("../src/renderer/src/stores/useBatchStore"))
    .useBatchStore;
});
...
describe("batch queue", () => {
  it("keeps serial order, resumes failed work, and resets checkpoints", async () => {
    useBatchStore.getState().addTasks([
      {
        filePath: "D:\\videos\\first.mp4",
        fileName: "first.mp4",
        duration: 30,
      },
      {
        filePath: "D:\\videos\\second.mp4",
        fileName: "second.mp4",
        duration: 40,
      },
    ]);
    ...
    useBatchStore
      .getState()
      .updateTask(first!.id, { status: "done", stageText: "完成" });
    ...
    const resumed = useBatchStore
      .getState()
      .tasks.find((task) => task.id === second!.id);
